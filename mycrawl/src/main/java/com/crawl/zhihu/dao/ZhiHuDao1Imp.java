...
public class ZhiHuDao1Imp implements ZhiHuDao1 {
	private static Logger logger = SimpleLogger.getSimpleLogger(ZhiHuDao1Imp.class);
	...
	public boolean insertUser(Connection cn, User u) {
		try {
			...
			pstmt.executeUpdate();
			pstmt.close();
			logger.info("插入数据库成功----" + u.getUserToken());
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
	...
	public boolean insertUrl(Connection cn, String md5Url) {
		...
		try {
			...
			String sql = "insert into url (md5_url) value (?)";
			PreparedStatement pstmt;
			pstmt = cn.prepareStatement(sql);
