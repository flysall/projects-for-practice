...
public class ZhiHuHttpClient extends AbstractHttpClient implements IHttpClient {
	...
	private void intiThreadPool() {
		detailPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize, Config.downloadThreadSize, 0L,
				TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(), "detailPageThreadPool");
		listPageThreadPool = new SimpleThreadPoolExecutor(50, 80, 0L, TimeUnit.MILLISECONDS,
				new LinkedBlockingQueue<Runnable>(5000), new ThreadPoolExecutor.DiscardPolicy(), "listPageThreadPool");
		...
		detailListPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize, Config.downloadThreadSize,
				0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(2000),
				new ThreadPoolExecutor.DiscardPolicy(), "detailListPageThreadPool");
	...
	public void manageHttpClient() {
		while (true) {
			...
			if (detailListPageThreadPool.isTerminated()) {
				...
				// 关闭代理检测线程池
				ProxyHttpClient.getInstance().getProxyTestThreadExecutor().shutdown();
				// 关闭代理下载页线程池
				ProxyHttpClient.getInstance().getProxyDownloadThreadExecutor().shutdown();

				break;
			}
			double costTime = (System.currentTimeMillis() - startTime) / 1000.0;
			logger.debug("抓取速度:" + parseUserCount.get() / costTime + "个/s");
			try {
				Thread.sleep(1000);
