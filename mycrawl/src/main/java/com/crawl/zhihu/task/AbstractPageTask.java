...
/**
 * page task 下载网页并解析，具体解析由子类实现 若使用代理，从ProxyPool中取出
 * 
 * @see ProxyPool
 */
public abstract class AbstractPageTask implements Runnable {
	...
	public void run() {
		...
		try {
			...
			String logStr = Thread.currentThread().getName() + " " + currentProxy + " executing request "
					+ page.getUrl() + "response statusCode:" + status + "request cost time:"
			...
			if (status == HttpStatus.SC_OK) {
				if (page.getHtml().contains("zhihu") && !page.getHtml().contains("安全验证")) {
					...
					currentProxy.setSuccessfulTotalTime(
							currentProxy.getSuccessfulTotalTime() + (requestEndTime - requestStartTime));
