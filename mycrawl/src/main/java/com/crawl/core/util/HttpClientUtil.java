...
public class HttpClientUtil {
	...
	private final static String userAgent = "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/527.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36";
...
//		InputStream fis = HttpClientUtil.class.getResourceAsStream(name);
		...
		try {
			...
			if (!file.exists() && isReplaceFile) {
				try {
					...
					is.close();
					os.close();
					logger.info(fileURL + "---文件下载成功至" + path + saveFileName);
				} catch (Exception e) {
					e.printStackTrace();
				}
			} else {
	...
	public static org.apache.http.client.config.RequestConfig.Builder getRequestConfigBuilder() {
		return RequestConfig.custom().setSocketTimeout(Constants.TIMEOUT).setConnectionRequestTimeout(Constants.TIMEOUT)
